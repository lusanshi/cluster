...
class Node(object):
    unique_ids = itertools.count() #节点编码

    def __init__(self, network, address):
        self.network = network
        self.address = address or 'N%d' % self.unique_ids.next() #此处应为next(iterator)
        self.logger = SimTimeLogger(logging.getLogger(self.address), 
    ...
    def receive(self, sender, message):
        handler_name = 'do_%s' % type(message).__name__ #获取消息类型
...
class Acceptor(Role):
    ...
    def __init__(self, node):
        super(Acceptor, self).__init__(node) #调用父类方法
    ...
    def do_Prepare(self, sender, ballot_num):
        ...
        self.node.send([sender], Promise(ballot_num=self.ballot_num,
...
class Member(object):
#每个节点对应一个Member类，通过invoke方法与集群交互
    ...
    def invoke(self, input_value, request_cls=Requester):
        assert self.requester is None #断言，判断为False时产生Assertionerror
        q = Queue.Queue()
